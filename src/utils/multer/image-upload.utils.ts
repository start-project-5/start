...
export const createStorage = (folder: string) =>
  diskStorage({
    ...
    // 2. Fayl nomini o'zgartirish (Folder nomini qo'shgan holda)
    filename: (_req, file, callback) => {
      const unique = `${Date.now()}-${Math.round(Math.random() * 1e9)}`;
      const fileExt = extname(file.originalname);
      
      // NOMNI SHU YERDA YOZAMIZ: folder-unikalID.kengaytma
      const fullFileName = `${folder}-${unique}${fileExt}`;
